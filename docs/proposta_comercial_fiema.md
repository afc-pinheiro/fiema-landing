...
### C. Integração Corporativa (Java + Spring Boot)
* **Backend de Integração**: Camada desenvolvida em Java (Spring Boot) responsável por fazer a ponte de comunicação segura e em tempo real entre o site público e os sistemas internos legados da FIEMA (tais como o SGE - Portal do Aluno, Portal de RH e sistemas de protocolo eletrônico).
* **Segurança e Auditoria**: Garante a criptografia e conformidade nas transações de dados de negócio, isolando o banco de dados interno da rede pública.
...
1. **Desenvolvimento do Frontend e do CMS**: Criação de todas as páginas públicas (Home, Sobre, Serviços, Notícias, Editais, Unidades e Contato) e configuração do Strapi.
2. **Desenvolvimento do Microsserviço de Integração (Java)**: Conexão das APIs e banco de dados de integração.
...
* **Acessos de API**: Documentação técnica e chaves de acesso (ambientes de homologação) dos sistemas internos integrados (SGE, RH, etc.).
* **Serviço de Envio (SMTP)**: Credenciais de envio de e-mail institucional (ou chaves de serviços como Amazon SES, Sendgrid, etc.) para configuração no Listmonk.
...
|---|---|---|---|
| **Fase 1: Setup & DevOps** | Mês 1 | Sprints 1 a 2 | Instalação do Coolify no servidor FIEMA, configuração de banco de dados, setup do Strapi CMS e Listmonk. Configuração da arquitetura base do Next.js. |
| **Fase 2: Estruturação & Migração** | Mês 2 | Sprints 3 a 4 | Implementação do script de importação e migração inicial de notícias, galerias e editais históricos para o Strapi. Integração básica do Listmonk. |
| **Fase 3: Frontend & Templates** | Mês 3 | Sprints 5 a 6 | Tradução dos designs visuais aprovados em código Next.js responsivo. Estruturação dinâmica das páginas de Notícias e Editais. |
| **Fase 4: Integração de APIs & Java** | Mês 4 | Sprints 7 a 8 | Desenvolvimento da camada de backend em Java (Spring) para comunicação e ponte com sistemas legados (SGE e RH). |
| **Fase 5: Métricas & Observabilidade** | Mês 5 | Sprints 9 a 10 | Conexão do Google Analytics (GA4), setup de dashboards de monitoramento de integridade e telemetria no Grafana, e auditoria interna de segurança. |
| **Fase 6: Homologação & Treinamento** | Mês 6 | Sprints 11 a 12 | Testes de aceitação com o cliente (UAT), treinamento de operação do Strapi e Listmonk, ajustes finos de layout, homologação e publicação final. |
...
2. **Parcela 2 — 30% (R$ 24.000,00)**: Faturada e devida ao **final da Sprint 4** (Mês 2), após a conclusão da migração automatizada das notícias e editais legados para o Strapi e validação dos dados importados.
3. **Parcela 3 (Entrega Final) — 40% (R$ 32.000,00)**: Faturada e devida na **homologação e entrega final das chaves do projeto** (final da Sprint 12), com o portal publicado no domínio definitivo, equipe treinada e observabilidade operacional ativa.
